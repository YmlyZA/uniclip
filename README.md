...
**Behind a reverse proxy (recommended).** [`deploy/`](deploy/README.md) has a VPS setup behind [Caddy](https://caddyserver.com) with automatic HTTPS — a self-contained `docker-compose.yml` (relay + Caddy) for a fresh host, and `vps-caddy.sh` to slot the relay into a Caddy you already run. **HTTPS is required off `localhost`** (the clipboard API needs a secure context). The relay derives its per-IP rate limits from the last `x-forwarded-for` hop, so **run it behind a proxy that sets a trustworthy client IP** (the provided Caddy config does); exposed directly, the header is spoofable. The image itself runs as a **non-root** user and ships a `HEALTHCHECK` that container orchestrators use to restart a wedged process.

**One-command updates.** After the first deploy (which writes the Caddy network to a git-ignored `deploy/relay.env`), update with `git pull && sudo ./deploy/update.sh` — a thin wrapper over `docker compose` that fills in `CADDY_NET` and `GIT_SHA` for you. A one-time `docker rm -f uniclip` hands the container off from the first run-based deploy; BuildKit caching keeps rebuilds fast; room URLs survive (only room *metadata* is persisted, on a volume). `vps-caddy.sh --update` is a run-based fallback — don't alternate the two. See [deploy/README.md → Updating](deploy/README.md#updating-a-running-deploy).
