...
- **DoS backstops are present, not absent.** The embedded relay (`lan-relay.ts`) caps concurrent peers (`maxPeers`, default 8) and rate-limits each socket with a sliding window (default 500 frames / 10 s, counted on raw inbound), plus the existing `MAX_FRAME_BYTES` drop — removing the unbounded socket-growth and per-socket fan-out-amplification vectors a malicious LAN peer could otherwise exploit. **Not** covered: connection *event*-rate abuse (rapid connect/disconnect churn produces `peer-joined`/`peer-left` broadcasts that aren't throttled) — a residual, bounded by `maxPeers` recipients and acceptable on an open LAN port. Perfect DoS resistance there is not a goal; the secret keeps content opaque regardless of any LAN-level abuse.
