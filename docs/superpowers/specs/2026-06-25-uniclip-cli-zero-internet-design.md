...
### Accepted limitations (LAN threat model — see `docs/security-review-2026-06-25.md`)
- **mDNS is unauthenticated**, so the `routingId → host:port` binding is spoofable: a malicious LAN peer can advertise the same `routingId` pointing at its own relay and hijack the rendezvous. This does **not** break confidentiality — the secret never leaves the QR, so a hijacked relay (like the legitimate one) only ever sees opaque ciphertext; the real host and joiner still derive the same key and E2EE each other through any relay. The residual risk is availability (an impostor can drop frames / refuse to forward signaling). Acceptable under the trusted-LAN model.
- **DoS backstops are present, not absent.** The embedded relay (`lan-relay.ts`) caps concurrent peers (`maxPeers`, default 8) and rate-limits each socket with two independent post-validate sliding windows — non-file frames (clips/signaling/presence) at 500 / 10 s and `file-*` frames at a higher 2000 / 10 s (so a relay-fallback file transfer isn't throttled; mirrors the public relay's `chunkLimiter`) — plus the existing `MAX_FRAME_BYTES` drop, removing the unbounded socket-growth and per-socket fan-out-amplification vectors. **Not** covered: connection *event*-rate abuse (rapid connect/disconnect churn produces `peer-joined`/`peer-left` broadcasts that aren't throttled), and invalid frames that fail schema validation (bounded by `MAX_FRAME_BYTES` + TCP backpressure, no fan-out) — residuals acceptable on an open LAN port, same as the public relay. Perfect DoS resistance there is not a goal; the secret keeps content opaque regardless of any LAN-level abuse.
- The mDNS instance name is a neutral `uniclip-<routingId>` — it does **not** carry the user's device name (which would leak in cleartext on the LAN). The device name still rides the **encrypted** presence channel only.
