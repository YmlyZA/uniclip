...
- **`apps/cli`** — a Node + **Ink** (React/JSX, ESM, built with **tsup** → `npx`-able `uniclip` bin) terminal UI that creates/joins a **Mode-A** room and syncs **text** over the relay, reusing `client-core`. **Relay-only**: P2P is disabled by injecting a never-opening `createConnection` stub (Node has no `RTCPeerConnection`); no persistence (in-memory session). `uniclip` mints a room (`--relay`/`UNICLIP_RELAY`, default `http://localhost:3000`) + shows a UTF-8 QR; `uniclip <room-url>` joins. Keybindings via Ink `useInput` (↑↓ select, c/⏎ copy → OS clipboard via `clipboardy`, q/Ctrl-C quit). Dev: `pnpm --filter @uniclip/cli dev`. Tests use `ink-testing-library` with an injected fake client (no pty/network). True zero-internet (mDNS + Node WebRTC + local signaling) is the deferred **P4b**.
